                ...
                for period in model.keys():
                    if model[period] and period in periods:
                        ...
                        if period == 'first':
                            ...
                            model[period]["price_fact"] = (
                                round(spend / approves, 2) if approves else 0
                            )

                            model[period]["price_max"] = round(
                                model[period]["usd_median"] * offer_kpi_percentage, 2
                            )

                            if approves == 0 and spend > 0:
                                model[period]["minus"] = -round(spend, 2)
                            elif model[period]["price_fact"] > model[period]["price_max"]:
                                model[period]["minus"] = round(
                                    (model[period]["price_max"] - model[period]["price_fact"]) * approves,
                                    2,
                                )
                            else:
                                model[period]["minus"] = 0
                ...
                if result.get(offer_id) and result[offer_id].get(buyer_id) and result[offer_id][buyer_id].get(site_type):
                    ...
                    for period in periods:
                        ...
                        if period == 'first':
                            ...
                            model[period]["price_fact"] = 0
                            model[period]["price_max"] = 0
                            model[period]["minus"] = -round(spend, 2) if spend > 0 else 0
