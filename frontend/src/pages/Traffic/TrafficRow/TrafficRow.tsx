    ...
    <TableRow className={isWhiteRow ? "whiteRow" : "defaultRow"}>
      ...
      {visibleColumns.includes("first.price_fact") && (
        <TableCell className={bg("purple")}>
          <PriceFactMaxCell fact={row.first.price_fact} max={row.first.price_max} />
        </TableCell>
      )}
      {visibleColumns.includes("first.minus") && (
        <TableCell align="right" className={bg("purple")} sx={{ color: (row.first.minus ?? 0) < 0 ? "#e53935" : undefined }}>
          {(row.first.minus ?? 0).toFixed(2)}$
        </TableCell>
      )}
    </TableRow>
  );
