...
import PriceFactMaxCell from "./PriceFactMaxCell/PriceFactMaxCell.tsx";
...
const Traffic = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(
    () => {
      const allKeys = trafficColumns.map(col => col.key);
      const saved = JSON.parse(localStorage.getItem("trafficVisibleColumns") || "null");
      if (!saved) return allKeys;
      const missing = allKeys.filter(key => !saved.includes(key));
  ...
  const summary = useMemo(() => {
    const result = displayedData.reduce(
      (acc, row) => {
        ...
        acc.price_max_weighted += (row.first.price_max ?? 0) * (row.first.approves_count ?? 0);
    ...
    const price_fact = result.approves_count > 0 ? round(spend / result.approves_count) : 0;
    const price_max = result.approves_count > 0
      ? round(result.price_max_weighted / result.approves_count)
      : 0;
    const minus = result.approves_count === 0 && spend > 0
      ? -spend
      : round(result.minus);
              ...
              >
                <TableRow sx={{ fontWeight: 600 }}>
                  ...
                  {visibleColumns.includes("first.price_fact") && (
                    <TableCell>
                      <PriceFactMaxCell fact={summary.price_fact} max={summary.price_max} />
                    </TableCell>
                  )}
                  {visibleColumns.includes("first.minus") && (
                    <TableCell align="right" sx={{ color: summary.minus < 0 ? "#e53935" : undefined }}>
                      {summary.minus.toFixed(2)}$
                    </TableCell>
                  )}
                </TableRow>
              </TableFooter>
